    ...
    def test_current_os_compatibility(self):
        ...
        try:
            ...
            with open(self.test_file, "r") as f1, open(decrypted_file, "r") as f2:
                self.assertEqual(
                    f1.read(), f2.read(), "Decrypted content should match original"
                )
            ...
            self.assertEqual(
                key,
                reconstructed_key,
                "Key reconstruction should work on the current OS",
            )
    ...
    def test_os_specific_path_handling(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                "Encrypted file should exist in nested directory",
            )
            ...
            self.assertTrue(
                ...
                "Decrypted file should exist in nested directory",
            )
            ...
            with open(test_file_in_subdir, "r") as f1, open(decrypted_file, "r") as f2:
                self.assertEqual(
                    f1.read(),
                    f2.read(),
                    "Decrypted content should match original in nested directory",
                )
            log_test_success(
                "Decrypted content identical to original in nested directory"
            )

            log_test_step(f"Using {platform.system()} path separator: {os.path.sep}")
            log_test_success(
                f"{platform.system()}-specific path handling tested successfully"
            )
    ...
    def test_current_python_version(self):
        ...
        try:
            ...
            try:
                ...
                log_test_success(
                    f"Encryption and decryption work on Python {python_version}"
                )
    ...
    def test_offline_operation(self):
        ...
        try:
            ...
            try:
                ...
                reconstructed_key = manager.combine_shares(shares[:3])
                self.assertEqual(
                    key, reconstructed_key, "Key reconstruction should work offline"
                )
                log_test_success("Secret sharing functional in offline mode")

                log_test_success(
                    "All operations completed successfully in offline mode"
                )
    ...
    def test_minimal_functionality(self):
        ...
        try:
            ...
            self.assertEqual(
                key,
                reconstructed_key,
                "Key reconstruction should work with minimal dependencies",
            )
            log_test_success("Key reconstruction successful with minimal dependencies")

            log_test_success(
                "Basic functionality operational with minimal dependencies"
            )
