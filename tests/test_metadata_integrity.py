...
class TestShareMetadata(unittest.TestCase):
    ...
    def test_metadata_creation(self):
        ...
        try:
            ...
            self.assertEqual(
                metadata.version, self.version, "Version not set correctly"
            )
            self.assertEqual(metadata.label, self.label, "Label not set correctly")
            self.assertEqual(
                metadata.threshold, self.threshold, "Threshold not set correctly"
            )
            self.assertEqual(
                metadata.total_shares,
                self.total_shares,
                "Total shares not set correctly",
            )
    ...
    def test_metadata_from_share_info(self):
        ...
        try:
            ...
            share_info = {
                ...
                "threshold": self.threshold,
                "total_shares": self.total_shares,
                "tool_integrity": {"shares_tool_version": self.version},
            }
            ...
            # Verify attributes
            self.assertEqual(
                metadata.version, self.version, "Version not loaded correctly"
            )
            self.assertEqual(metadata.label, self.label, "Label not loaded correctly")
            self.assertEqual(
                metadata.threshold, self.threshold, "Threshold not loaded correctly"
            )
            self.assertEqual(
                metadata.total_shares,
                self.total_shares,
                "Total shares not loaded correctly",
            )
    ...
    def test_metadata_to_dict(self):
        ...
        try:
            ...
            # Verify dictionary
            self.assertEqual(
                metadata_dict["version"], self.version, "Version not in dictionary"
            )
            self.assertEqual(
                metadata_dict["label"], self.label, "Label not in dictionary"
            )
            self.assertEqual(
                metadata_dict["threshold"],
                self.threshold,
                "Threshold not in dictionary",
            )
            self.assertEqual(
                metadata_dict["total_shares"],
                self.total_shares,
                "Total shares not in dictionary",
            )
...
class TestIntegrityVerification(unittest.TestCase):
    ...
    def test_tool_integrity_calculation(self):
        ...
        try:
            ...
            self.assertIn(
                "packages_hash", integrity, "Packages hash missing from integrity"
            )
            self.assertIn(
                "shares_tool_version", integrity, "Tool version missing from integrity"
            )
            ...
            # Verify version
            self.assertEqual(
                integrity["shares_tool_version"], VERSION, "Tool version incorrect"
            )
            ...
            self.assertIsInstance(
                integrity["tool_hash"], str, "Tool hash should be a string"
            )
            self.assertTrue(
                len(integrity["tool_hash"]) > 0, "Tool hash should not be empty"
            )
            ...
            self.assertIsInstance(
                integrity["packages_hash"], dict, "Packages hash should be a dictionary"
            )
            log_test_success(
                f"Valid packages hash with {len(integrity['packages_hash'])} packages"
            )
    ...
    def test_tampered_share_detection(self):
        ...
        try:
            ...
            tampered_file = os.path.join(
                self.temp_dir.name, f"tampered_share_{idx}.json"
            )
            ...
            tampered_share_data[corrupt_pos] = (
                tampered_share_data[corrupt_pos] + 1
            ) % 256
    ...
    def test_share_loading_with_integrity_check(self):
        ...
        try:
            ...
            for idx, share_data in shares[
                : self.threshold
            ]:  # Just use threshold number of shares
                # Calculate share hash
                share_hash = hashlib.sha256(share_data).hexdigest()
            ...
            self.assertEqual(
                len(loaded_shares), self.threshold, "Incorrect number of shares loaded"
            )
            ...
            tampered_file = os.path.join(
                self.temp_dir.name, f"tampered_share_{tampered_idx}.json"
            )
            ...
            tampered_info["label"] = (
                self.label + "_tampered"
            )  # Modify the label to ensure a mismatch
            ...
            with self.assertRaises(
                ValueError, msg="Loading tampered shares should raise ValueError"
            ):
            ...
            tampered_share_data = bytearray(original_share_data)
            tampered_share_data[0] = (
                tampered_share_data[0] + 1
            ) % 256  # Modify first byte
            tampered_hash_info["share_key"] = base64.b64encode(
                tampered_share_data
            ).decode()
