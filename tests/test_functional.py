...
class CLIEndToEndTests(unittest.TestCase):
    ...
    def test_different_threshold_combinations(self):
        ...
        for threshold, total_shares in combinations:
            with self.subTest(
                f"Combination: threshold={threshold}, shares={total_shares}"
            ):
                log_step(
                ...
                # Create shares manually
                log_step(
                    f"Creating shares with threshold={threshold}, total_shares={total_shares}"
                )
                manager = ShareManager(threshold, total_shares)
                shares = manager.generate_shares(test_key, label)
                log_success(f"Generated {len(shares)} shares")
                ...
                for idx, share_data in shares:
                    share_file = (
                        self.shares_dir
                        / f"combo_{threshold}_{total_shares}_share_{idx}.txt"
                    )
                ...
                loaded_shares, metadata = ShareManager.load_shares(
                    share_files[:threshold]
                )

                # Verify number of loaded shares
                self.assertEqual(
                    len(loaded_shares), threshold, "Incorrect number of shares loaded"
                )
                ...
                # Verify key reconstruction
                self.assertEqual(
                    reconstructed_key, test_key, "Failed to reconstruct key"
                )
                log_success("Successfully reconstructed the original key")
                ...
                if total_shares > threshold:
                    ...
                    random_reconstructed_key = manager.combine_shares(
                        random_loaded_shares
                    )

                    # Verify random selection reconstruction
                    self.assertEqual(
                        random_reconstructed_key,
                        test_key,
                        "Failed to reconstruct key with random selection of shares",
                    )
                    log_success(
                        "Successfully reconstructed key with random share selection"
                    )
    ...
    def test_archive_creation_extraction(self):
        ...
        label = "archive_test"
        log_info(
            f"Using parameters: threshold={threshold}, total_shares={total_shares}, label={label}"
        )
        ...
        for expected_file in expected_files:
            ...
            self.assertTrue(
                ...
                f"Expected file {expected_file} not found in archive",
            )
        ...
        self.assertTrue(
            (src_dir / "__init__.py").exists(), "source code not found in archive"
        )
    ...
    def test_interactive_mode(self):
        ...
        self.assertEqual(
            reconstructed_key,
            test_key,
            "Failed to reconstruct key in interactive mode simulation",
        )
        log_success("Successfully reconstructed the original key")
...
class CompatibilityTests(unittest.TestCase):
    ...
    def test_version_compatibility(self):
        ...
        for i in range(threshold):
            ...
            if i == 0:
                ...
                share_info = {
                    ...
                    "total_shares": total_shares,
                    "version": current_version,
                    "tool_integrity": {"shares_tool_version": current_version},
                }
            else:
        ...
        try:
            ...
            log_success(
                f"Successfully loaded {len(loaded_shares)} shares with mixed formats"
            )
            ...
            self.assertEqual(
                reconstructed_secret[: len(test_secret)],
                test_secret,
    ...
    def test_cross_platform_compatibility(self):
        ...
        for i, (idx, share_data) in enumerate(shares[:threshold]):
            ...
            share_info = {
                ...
                "platform_path": (
                    f"C:{platform_separator}path{platform_separator}to{platform_separator}share"
        ...
        log_success(
            f"Created {len(platform_shares)} shares with platform-specific paths"
        )
        ...
        try:
            ...
            loaded_shares, metadata = ShareManager.load_shares(
                [str(s) for s in platform_shares]
            )
            ...
            log_success(
                f"Successfully loaded {len(loaded_shares)} shares from different platforms"
            )
...
class ErrorHandlingTests(unittest.TestCase):
    ...
    def test_corrupted_shares(self):
        ...
        with self.assertRaises(
            Exception, msg="Should fail when loading corrupted JSON"
        ):
    ...
    def test_insufficient_shares(self):
        ...
        try:
            shares_data, metadata = ShareManager.load_shares(
                [str(s) for s in insufficient_shares]
            )
    ...
    def test_file_access_issues(self):
        ...
        with self.assertRaises(
            (FileNotFoundError, ValueError), msg="Should fail with non-existent file"
        ):
    ...
    def test_incorrect_keys(self):
        ...
        # Load wrong shares
        wrong_shares_data, metadata = ShareManager.load_shares(
            [str(s) for s in wrong_share_files]
        )
        ...
        with self.assertRaises(
            ValueError, msg="Should fail with incorrect reconstructed key"
        ):
            decrypted_file = self.test_dir / "wrong_reconstructed_decrypted.txt"
            wrong_key_encryptor.decrypt_file(
                str(self.encrypted_file), str(decrypted_file)
            )
